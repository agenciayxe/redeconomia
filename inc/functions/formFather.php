<?php 
add_action('wp_ajax_nopriv_father', 'father');
add_action('wp_ajax_father', 'father');
...
function father() {
    ...
    else if ($content['email'] == '') { $response['content'] = 'O documento digitado não é válido.'; }
    ...
    else if (strlen($content['email']) <= 4) { $response['content'] = 'O documento é muito pequeno.'; }
    else if (strlen($content['phone']) <= 7) { $response['content'] = 'O telefone é muito pequeno.'; }
   /* else if (strlen($content['message']) <= 30) { $response['content'] = 'A mensagem está muito pequena.'; }*/
    ...
    if(!in_array($extFile, $typeAccepts) && !empty($_FILES['photo']) && $_FILES['photo']['tmp_name'] != '') { $verifyFields = false; $response['content'] = 'Insira a foto com a sua pai para continuar.'; }
    ...
    if ($verifyFields) {
        ...
        if ($insertCustomer) {
            ...
            if (!empty($_FILES['photo']) && $_FILES['photo']['tmp_name']) {
                ...
                $pathUpload = (!empty($_FILES['photo'])) ? get_home_path() . '/pais/' . $idCustomer .  '-' . $slugTitle . '.' . $extFile : '';
                $urlUpload = (!empty($_FILES['photo']) || $_FILES['photo']['tmp_name'] == '') ? 'https://www.redeconomia.com.br/pais/' . $idCustomer .  '-' . $slugTitle . '.' . $extFile : '';
